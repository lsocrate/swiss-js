...
test("Get match matrix", function () {
  ...
  equal(matrix.matches.size(), 9)
  ok(matrix.matches["mp1@p3"])
  ok(matrix.matches["mp1@p4"])
  ok(matrix.matches["mp1@p5"])
  ok(matrix.matches["mp2@p3"])
  ok(matrix.matches["mp2@p4"])
  ok(matrix.matches["mp2@p5"])
  ok(matrix.matches["mp3@p4"])
  ok(matrix.matches["mp3@p5"])
  ok(matrix.matches["mp4@p5"])
})
...
test("Get unique matches which are the only possible for some of it's players", function () {
  ...
  var matrix = this.tournament.getMatchMatrixForPlayers(this.players)
  equal(matrix.matches.size(), 7)
  var singularMatches = matrix.getSingularMatches()
  equal(singularMatches.size(), 1)
})